...
Public Class SpanTrainDriveLength : Inherits Report
    ...
    Public Overrides Sub LoadData()
        ...
        If Me.ReportType = ReportStyle.空车 Then
            ...
            Exit Sub
        Else
            str = "select t.traincode,sum(t.drivelength) as drivelength,t.dutydate from (select t.vehicleno as traincode,t.distance as drivelength,m.dateno as dutydate from cs_crewschedule t,cs_corresponding m,cs_datetimetable x " & _
    ...
    Public Overrides Sub DrawFrame()
        ...
        Me.DataGrid.Grid(0, Me.DataGrid.columns - 2).celltext = "总运营里程"
        Me.DataGrid.Grid(0, Me.DataGrid.columns - 1).celltext = "总走行公里"
    ...
    Public Overrides Sub DrawData()
        ...
        Me.table.Dispose()
        Dim str As String = "select t.traincode,sum(t.drivelength) as peoplelength,'' as emptylength,t.dutydate from (select t.vehicleno as traincode,t.distance as drivelength,m.dateno as dutydate from cs_crewschedule t,cs_corresponding m,cs_datetimetable x " & _
                    "where datediff('d',m.dateno,x.dateno)=0 and m.driverno=t.driverno and x.cstimetableid=t.cstimetableid and " & _
                    "datediff(m.dateno,Format('" & StartReportDate.ToString("yyyy/MM/dd") & "','yyyy/MM/dd'))<=0 and datediff('d',m.dateno,Format('" & EndReportDate.ToString("yyyy/MM/dd") & "','yyyy/MM/dd'))>=0 order by t.vehicleno,t.endtime) t group by t.dutydate,t.traincode order by t.traincode,t.dutydate"
        Dim tab As New System.Data.DataTable
        tab = Globle.Method.ReadDataForAccess(str)


        tab.Columns("traincode").ColumnName = "列车号"
        tab.Columns("peoplelength").ColumnName = "载客公里"
        tab.Columns("emptylength").ColumnName = "空车公里"
        tab.Columns("dutydate").ColumnName = "日期"
        ...
        For i As Integer = 0 To tab.Rows.Count - 1
            If i = 0 Then
                Me.DataGrid.Grid(point * 2 + 2, 1 + (CDate(tab.Rows(i).Item("日期")).AddDays(1) - Me.StartReportDate).Days).celltext = tab.Rows(i).Item("载客公里").ToString
                Me.DataGrid.Grid(point * 2 + 3, 1 + (CDate(tab.Rows(i).Item("日期")).AddDays(1) - Me.StartReportDate).Days).celltext = tab.Rows(i).Item("空车公里").ToString
                If tab.Rows(i).Item("载客公里").ToString = "" Then
                    sum1 += 0
                Else
                    sum1 += CDec(tab.Rows(i).Item("载客公里"))
                End If
                If tab.Rows(i).Item("空车公里").ToString = "" Then
                    sum2 += 0
                Else
                    sum2 += CDec(tab.Rows(i).Item("空车公里"))
                End If

            Else
                If tab.Rows(i).Item("列车号") <> tab.Rows(i - 1).Item("列车号") Then
                    Me.DataGrid.Grid(point * 2 + 2, Me.DataGrid.columns - 2).celltext = sum1.ToString()
                    Me.DataGrid.Grid(point * 2 + 3, Me.DataGrid.columns - 2).celltext = sum2.ToString()
                    Me.DataGrid.Grid(point * 2 + 2, Me.DataGrid.columns - 1).celltext = (sum1 + sum2).ToString()
