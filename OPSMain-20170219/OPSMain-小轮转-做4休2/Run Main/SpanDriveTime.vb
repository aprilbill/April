...
Public Class SpanDriveTime : Inherits Report
    ...
    Public Overrides Sub DrawFrame()
        ...
        Me.DataGrid = New CellGrid(4, 1, DriverNum + 2, Me.DateSpan.Days + 4)
        Me.DataGrid.Grid(0, 0).celltext = "线路编号"
        Me.DataGrid.Grid(0, 1).celltext = "司机编号"
        Me.DataGrid.Grid(0, 2).celltext = "司机姓名"
        Me.DataGrid.Grid(0, 3).celltext = "日驾驶工时"
        Me.DataGrid.Grid(0, Me.DataGrid.columns - 1).celltext = "总驾驶工时"
            ...
            For Each rdr As RDriver In team
                If rdr.CSDriverList.Count > 0 Then
                    ...
                    For i As Integer = 0 To rdr.CSDriverList.Count - 1
                        Me.DataGrid.Grid(point + 2, 3 + i).celltext = Coordination2.Global.BeTime(rdr.CSDriverList(i).TotalWorkTime)
                    Next
                    Me.DataGrid.Grid(point + 2, Me.DataGrid.columns - 1).celltext = Coordination2.Global.BeTime(rdr.TotalWorkTime)
                    point += 1
                Else
                    Exit Sub
                End If
