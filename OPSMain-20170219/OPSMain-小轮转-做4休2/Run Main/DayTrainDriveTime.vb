...
Public Class DayTrainDriveTime : Inherits Report
    ...
    Public Overrides Sub LoadData()
        ...
        If Me.ReportType = ReportStyle.空车 Then
            ...
            Exit Sub
        Else
            str = "select t.vehicleno as traincode,iif((val(t.endtime)-val(t.starttime))<0 ,(val(t.endtime)-val(t.starttime))+86400 ,(val(t.endtime)-val(t.starttime))) as drivetime from cs_crewschedule t,cs_corresponding m,cs_datetimetable x " & _
    ...
    Public Overrides Sub DrawFrame()
        ...
        Dim Maxdutynum As Integer = 0
        If temptab.Rows.Count > 0 Then
            Maxdutynum = temptab.Rows(0).Item(0)
        End If
        ...
        Me.DataGrid.Grid(0, Me.DataGrid.columns - 2).celltext = "日运营时间"
        Me.DataGrid.Grid(0, Me.DataGrid.columns - 1).celltext = "日走行时间"
    ...
    Public Overrides Sub DrawData()
        ...
        Dim str As String = "select t.vehicleno as traincode,iif((val(t.endtime)-val(t.starttime))<0,(val(t.endtime)-val(t.starttime))+86400,(val(t.endtime)-val(t.starttime))) as drivetime,'' as isempty from cs_crewschedule t,cs_corresponding m,cs_datetimetable x " & _
                        "where datediff('d',m.dateno,x.dateno)=0 and m.driverno=t.driverno and x.cstimetableid=t.cstimetableid and " & _
                        "datediff('d',m.dateno,Format('" & ReportDate.ToString("yyyy/MM/dd") & "','yyyy/MM/dd'))=0 order by t.vehicleno,t.endtime"
        ...
        For i As Integer = 0 To tab.Rows.Count - 1
            If i = 0 Then
                If tab.Rows(i).Item("是否空载").ToString = "是" Then
                    count2 += 1
                    Me.DataGrid.Grid(point * 2 + 3, count2 + 1).celltext = Coordination2.Global.BeTime(CInt(tab.Rows(i).Item("运营时间")))
                    tempsum2 += CInt(tab.Rows(i).Item("运营时间"))
                Else
                    count += 1
                    Me.DataGrid.Grid(point * 2 + 2, count + 1).celltext = Coordination2.Global.BeTime(CInt(tab.Rows(i).Item("运营时间")))
                    tempsum += CInt(tab.Rows(i).Item("运营时间"))
                End If
            Else
                If tab.Rows(i).Item("列车号") = tab.Rows(i - 1).Item("列车号") Then
                    ...
                    End If
                Else
                    Me.DataGrid.Grid(point * 2 + 2, Me.DataGrid.columns - 2).celltext = Coordination2.Global.BeTime(tempsum)
                    Me.DataGrid.Grid(point * 2 + 3, Me.DataGrid.columns - 2).celltext = Coordination2.Global.BeTime(tempsum2)
                    Me.DataGrid.Grid(point * 2 + 2, Me.DataGrid.columns - 1).celltext = Coordination2.Global.BeTime(tempsum + tempsum2)
                    point += 1
                    count = 0
                    count2 = 0
                    tempsum = 0
